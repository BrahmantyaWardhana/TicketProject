	...
	public Login() {
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String query = "SELECT * FROM bwardhana_users WHERE uname = ? and upass = ?;";
				try (PreparedStatement stmt = conn.getConnection().prepareStatement(query)) {
					...
					if (rs.next()) {
                        uname = rs.getString("uname"); // get username
						admin = rs.getBoolean("admin"); // get table column value
						new Tickets(admin, uname); //open Tickets file / GUI interface
